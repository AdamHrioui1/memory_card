...
let displayCards = (shuffled_array, layout_size) => {
    ...
    for (let i = 0; i < shuffled_array.length; i++) {
        ...
        if(layout_size === 4) card.classList.add('four')
        if(layout_size === 6) card.classList.add('six')
...
let selectCard = (e) => {
    ...
    if(score % 2 === 0) {
        if(isCardsMatches(two_selected_cards[0], two_selected_cards[1], layout_size)) {
...
let isCardsMatches = (card1, card2, layout_size) => {
    if(layout_size === 4) return matches_4x4[card1] === card2 
    else return matches_6x6[card1] === card2 
}

let youWin = (time) => {
    if(matched_cards_counter === layout_size**2 / 2) {
